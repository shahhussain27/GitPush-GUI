...
const CollaborationView: React.FC<CollaborationViewProps> = ({
  ...
  githubPat, setGithubPat, deleteGithubPat
}) => {
  const [localToken, setLocalToken] = useState(githubPat || '')

  useEffect(() => {
    setLocalToken(githubPat || '')
  }, [githubPat])
  ...
  const handleConnect = async () => {
    const activeToken = githubPat || localToken
    if (!activeToken || !owner || !repo) {
      setMessage('⚠️ Please provide PAT, Owner, and Repo name.')
      return
    }

    if (activeToken !== githubPat && setGithubPat) {
      await setGithubPat(activeToken)
    }

    setIsLoading(true)
    setMessage('')
    try {
      const details = await onGetRepoDetails(activeToken, owner, repo)
      if (details.success) {
        setIsOrg(details.details.owner.type === 'Organization')
        const collabs = await onListCollaborators(activeToken, owner, repo)
        if (collabs.success) {
  ...
  const handleDisconnectPat = async () => {
    if (confirm('Are you sure you want to forget your saved Personal Access Token?')) {
      if (deleteGithubPat) await deleteGithubPat()
      setLocalToken('')
      setCollaborators([])
      setMessage('✅ PAT disconnected.')
    }
  }

  const handleAddCollaborator = async () => {
    if (!newUsername || !githubPat) return
    setIsLoading(true)
    const result = await onAddCollaborator(githubPat, owner, repo, newUsername, permission)
    if (result.success) {
  ...
  const handleRemoveCollaborator = async (username: string) => {
    if (!confirm(`Remove ${username} from repository?`) || !githubPat) return
    setIsLoading(true)
    const result = await onRemoveCollaborator(githubPat, owner, repo, username)
    if (result.success) {
  ...
  const handleAddTeam = async () => {
    if (!teamSlug || !githubPat) return
    setIsLoading(true)
    const result = await onAddTeamRepo(githubPat, owner, teamSlug, owner, repo, permission)
    if (result.success) {
        ...
        <CardContent className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-3">
              ...
              {githubPat ? (
                <div className="flex items-center justify-between bg-green-500/10 border border-green-500/20 rounded-2xl p-3 px-4">
                  <div className="flex items-center gap-2">
                    <Shield className="size-4 text-green-500" />
                    <span className="text-sm font-bold text-green-400">PAT Connected</span>
                  </div>
                  <Button variant="ghost" size="sm" onClick={handleDisconnectPat} className="text-[10px] uppercase font-black tracking-widest text-red-400 hover:text-red-300 hover:bg-red-500/10 h-7 rounded-lg px-2">
                    Disconnect
                  </Button>
                </div>
              ) : (
